...
class Thread(threading.Thread):
    def __init__(self,threadID, name, scripts, duration_time, think_time):
        ...
        self.db_con = conn_db.conn_db(db_type,db_user,db_pwd,db_address,db_port,db_name)
    def run(self):
        print "Start " + self.name
        self.execute_sql(self.scripts, self.think_time, self.duration_time,self.name)
        self.db_con.commit()
        print "End " + self.name

    def execute_sql(self, scripts, think_time, duration_time, thread_name):
        path = "./script/"+scripts
